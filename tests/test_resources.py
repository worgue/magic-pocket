...
def test_secretsmanager(use_toml):
    ...
    assert settings.container["main"]
    settings.container["main"].secrets = Secrets(
...
def test_context_from_toml_does_not_call_aws(use_toml, monkeypatch):
    ...
    assert context.container["main"].secrets.managed
...
def test_initial_secretsmanager_policy(use_toml):
    ...
    assert context.container["main"]
    assert context.container["main"].secrets
    assert context.container["main"].secrets.allowed_sm_resources == []
...
def test_initial_ssm_policy(use_toml):
    ...
    assert context.container["main"]
    assert context.container["main"].secrets
    assert context.container["main"].secrets.store == "ssm"
    # SSMのmanagedなので、allowed_ssm_resourcesにパターンが入る
    assert context.container["main"].secrets.allowed_ssm_resources != []
    # SMリソースはない
    assert context.container["main"].secrets.allowed_sm_resources == []
...
def test_ssm_pocket_secrets(use_toml):
    ...
    mediator = Mediator(context)
    mediator.ensure_pocket_managed_secrets()
    sc = context.container["main"].secrets
