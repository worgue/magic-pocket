...
```toml
# devではAPI Gatewayデフォルトのurl、prodではexample.comを利用します。
...
```toml
# prodデプロイ時に、example.comからwww.example.comのリダイレクトを追加する設定
[prod.spa]
...
```toml
# prodとdevのLambda環境で別のDEFAULT_FROM_EMAILを設定
