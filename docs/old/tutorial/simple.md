...
9.  `dev`と`prod`のwsgiにapigatewayが設定されます。URLはAWS側で自動生成されます。項目を分けているのは、`apigateway = { domain = "example.com" }`のように、ドメインを指定をするためです。Route53のホストゾーンがあれば、自動設定可能です。外部の設定を用いる場合、CloudFomationがDNSレコードの認証待ちになります。
